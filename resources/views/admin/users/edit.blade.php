        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        ...
                        <select name="role" id="role" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->name }}" 
                                    {{ (old('role') ? old('role') : ($user->hasRole($role->name) ? $role->name : '')) == $role->name ? 'selected' : '' }}>
                                    {{ ucfirst($role->name) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <h3 class="text-sm font-semibold text-gray-600 mb-4">Ganti Password (Opsional)</h3>
                    ...
                    <div class="flex items-center justify-end mt-4 gap-6">
                        ...
                        &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
                        <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-black font-bold py-2 px-4 rounded shadow-sm text-sm">
                            Update User
                        </button>
                    </div>
                </form>
